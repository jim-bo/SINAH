...
class MstGraph(object):
    ...
    def __init__(self, successors, get_score=None, get_label=None):
        '''Initialize this MstGraph using a successors map and a score function.
    ...
    def __contains__(self, x):
        '''Return True iff x is a node in our MstGraph.'''
    ...
    def num_nodes(self):
        '''Return the number of nodes in this MstGraph.'''
        return len(self.successors)

    def num_edges(self):
        '''Return the number of edges in this MstGraph.'''
    ...
    def dot(self, name):
        ...
        return 'MstGraph _%s {%s %s}' % (name, nodes, edges)

    def iteredges(self):
        '''Iterate over the pairs of node ids in all edges in this MstGraph.'''
        for source, targets in self.successors.iteritems():
            for target in targets:
                yield source, target

    def mst(self):
        '''Return the MST of this MstGraph using the Chu-Liu-Edmonds algorithm.

        Returns a new MstGraph.
        '''
        mark = MstGraph.new_node_id
    ...
    def find_cycle(self):
        '''Find and return a cycle in our MstGraph, or None.'''
        # from guido's blog :
            ...
            while stack:
                ...
                for node in self.successors.get(top, ()):
                    try:
                        ...
                        return MstGraph(succs, self.get_score, self.get_label)
                    except ValueError:
                        pass
                    if node in worklist:
                        stack.append(node)
    ...
    def contract(self, cycle):
        ...
        Returns a tuple (id, graph). The graph is a new MstGraph instance
        ...
        new_id = MstGraph.new_node_id
        MstGraph.new_node_id += 1
        ...
        for target, sources in from_cycle.iteritems():
        ...
        for source, targets in to_cycle.iteritems():
        ...
        return new_id, old_edges, MstGraph(succs,
    ...
    def merge(self, mst, new_id, old_edges, cycle):
        ...
        mst: A MstGraph containing an MST of our graph, with a single node
        ...
        cycle: A MstGraph containing nodes and edges in a cycle of our graph.

        Return a new MstGraph containing the merged nodes and edges.
        '''
        ...
        for source, target in mst.iteredges():
            if source == new_id:
               ...
               # logging.debug('%s -> %s: cycle -> mst', source, target)
                for s, ts in old_edges.iteritems():
                    for t in ts:
                        if t == target:
                            succs[s].append(t)
    ...
    def greedy(self):
        '''Return a MstGraph consisting of the max scoring edge for each node.'''
        ...
        succs = dict((n, []) for n in self)
        for target, source in max_sources.iteritems():
            succs[source].append(target)
        return MstGraph(succs, self.get_score, self.get_label)
