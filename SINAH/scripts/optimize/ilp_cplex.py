	...
	def _constrain_path(self):
		...
		for i in range(self._blist.size):
			...
			if self._blist[i]['idxa'] not in self._neibs:
				neibs[self._blist[i]['idxa']] = set()
			if self._blist[i]['idxb'] not in self._neibs:
				neibs[self._blist[i]['idxb']] = set()

			# track.
			neibs[self._blist[i]['idxa']].add(self._blist[i]['idxb'])
			neibs[self._blist[i]['idxb']].add(self._blist[i]['idxa'])
